    ...
    return (
        <div className="flex flex-col sm:flex-row gap-3">
            <form onSubmit={handleSearch} className="relative flex-1">
                <Search className="absolute left-3.5 h-4 w-4 text-sand-400 pointer-events-none top-1/2 -translate-y-1/2" />
                <input
                    type="text"
                    value={search}
            ...
            <button
                ...
                className={`flex items-center gap-1.5 rounded-lg border px-3.5 py-2 text-sm font-medium shadow-sm transition-colors disabled:opacity-50 ${
                    atRiskOnly ? 'border-status-critical-bg bg-status-critical-bg text-status-critical' : 'border-sand-200 bg-white text-ink-700 hover:bg-sand-50'
                }`}
            >
